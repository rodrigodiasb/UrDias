...
function renderEval(app, dayId, evId){
    ...
    <main class="content">
      ...
      ${section("2) Dados pessoais", `
        ...
        ${field("CPF ou Documento", `<input class="input" id="doc" inputmode="numeric" pattern="[0-9]*" placeholder="Digite apenas números" />`,
          `<span id="docHint" class="hint">Aceita apenas números.</span>`
        )}
        <div class="grid2">
          <div class="card">
  ...
  $("#doc").addEventListener("input", e=>{
    const digits = onlyDigits(e.target.value).slice(0,30);
    e.target.value = digits;
    if(digits.length===11 && isValidCPF(digits)){
      $("#docHint").textContent = "Detectado: CPF válido.";
      apply(n=>{ n.pessoa.documento=digits; n.docTipo="cpf"; });
    }else{
      $("#docHint").textContent = "Aceita apenas números.";
  ...
  $("#senha").addEventListener("input", e=>{
    const valor = sanitizeInteger(e.target.value, 30);
    e.target.value = valor;
    apply(n=>{ n.regulacao.senha=valor; });
  });
